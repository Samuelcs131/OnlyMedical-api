...
export default {
    ...
    Query: {
        ...
        getExamPulmonaryWithInterval: async (_, {dataInicial, dataFinal, CPF}) => {
            ...
            let getExamInterval = (await indice_pulmonar).slice(dateInitial, dateFinal)
            const resultSearch = getExamInterval.map( exam => ( {data: exam.data, exame: exam.dados.filter( patient => patient.CPF === CPF)} ))
            return resultSearch
        },

        getExamPulmonaryWithIntervalValue: async (_, {valorInicial, valorFinal, CPF}) => { return ((await indice_pulmonar).map( exam => { 
            return ({ 
                data: exam.data, 
